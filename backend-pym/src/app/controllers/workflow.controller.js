    ...
    camposArray.forEach((campo) => {
      if (req.body[campo] && typeof req.body[campo] === "string") {
        try {
          ...
          console.warn(
            `No se pudo parsear el campo ${campo} como JSON:`,
            req.body[campo],
          );
    ...
    switch (transaccion) {
      ...
      case "OBSERVAR":
        datosCorreo = [
          ...
          "Estimado(a) " + req.body.nombres + " " + req.body.primer_apellido,
    ...
    if (user.estado_wf === "ACTIVO") {
      ...
      if (resp.accepted.length > 0) {
        res.status(200).json({
          notificacion:
            ...
            user.correo +
            ", ingresa a tu cuenta para poder restablecer tu contraseña.",
          codigoresultado: 200,
        });
      } else {
        res.status(400).json({
      ...
      res.status(200).json({
        result: {
          correcto: false,
          notificacion:
            "El usuario no está activo, estado actual: " +
            user.estado_wf +
            ", contacta a soporte técnico si crees que es un error.",
          codigoresultado: 400,
          datoadicional: null,
        },
      });
  ...
  if (resultadoDesencriptado.expirado) {
    return res.status(200).json({
      result: {
        correcto: false,
        notificacion:
          "El enlace ha expirado. Por favor, solicita un nuevo enlace de restablecimiento.",
        codigoresultado: 400,
        datoadicional: null,
      },
    });
